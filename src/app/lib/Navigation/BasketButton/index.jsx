...
export function BasketButton() {
    ...
    const calculateNumberOfItems = (basket) => {
        if (!basket || !basket.products) {
            return 0;
        }

        return basket.products.reduce((total, product) => total + product.quantity, 0);
    };

    useEffect(() => {
        setNumberOfItems(calculateNumberOfItems(basket));
    }, [basket]);
    ...
    return (
        <Link href='/basket' className={navigationStyles.link}>
            {/* <svg viewBox="0 0 24 24" className={clsx(styles.basket)}>
